...
flutter analyze
flutter test
flutter test --coverage   # Generate coverage report; required ≥89% for lib/
```

**Test coverage:** Coverage for `lib/` must be at least 89%. Run `flutter test --coverage` before submitting changes.
...
| Tests fail | Run `flutter test --coverage`; ensure coverage ≥89% for new code |
